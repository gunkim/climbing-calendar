...
import { ChevronLeft, ChevronRight, Plus, CalendarIcon, MapPin, Clock } from "lucide-react"
import {
  format,
  addMonths,
  subMonths,
                ...
                {hasEvents && (
                  <div className="mt-1 space-y-1">
                      ...
                      return (
                        <TooltipProvider key={event.id}>
                          <Tooltip>
                            ...
                            <TooltipContent className="p-3 max-w-xs bg-white border border-gray-200 shadow-md">
                    ...
                    <div className="mt-1 overflow-hidden">
                      {(() => {
                        // 카운트가 있는 난이도만 필터링
                        const visibleDifficulties = allDifficultiesForDay.filter(
                          (color) => difficultyCounts[color.name] && difficultyCounts[color.name] > 0,
                        )
                        ...
                        return (
                          <div className="flex flex-wrap gap-1">
                            {displayDifficulties.map((color) => {
                              const count = difficultyCounts[color.name]
                              return (
                                <div
                                  key={color.name}
                                  className={`rounded-full px-1.5 py-0.5 text-xs font-medium ${color.bgColor} ${color.textColor} border ${color.borderColor}`}
                                >
                            ...
                            {remainingCount > 0 && (
                              <div className="rounded-full px-1.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">
                                +{remainingCount}
                              </div>
                            )}
                          </div>
