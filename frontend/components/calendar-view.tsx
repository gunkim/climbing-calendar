...
import {GetScheduleResponse} from "@/apis/schedules";
...
export function CalendarView() {
    ...
    const {schedules, addSchedule, updateSchedule, deleteSchedule} = useSchedules(currentDate)
    ...
    const getEventsForDay = (day: Date) => {
        return schedules.filter((event) => isSameDay(new Date(event.scheduleDate), day))
    }
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[425px] bg-white border-gray-200">
                    ...
                    <EventForm
                        ...
                        onSubmit={(eventData) => {
                            if (selectedEvent) {
                                updateSchedule(selectedEvent.id, eventData)
                            } else {
