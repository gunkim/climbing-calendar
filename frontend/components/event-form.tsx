...
export function EventForm({selectedDate, selectedEvent, onSubmit, onDelete}: EventFormProps) {
    ...
    selectedEvent?.clearList.forEach((clear) => {
        const key = clear.id
        defaultDifficulties[key] = clear.count
    })
    ...
    const handleSubmit = (data: any) => {
        ...
        onSubmit({
            ...
            clearList: Object.entries(data.levelId).map(([key, value]: [string, any]) => ({
                levelId: Number(key),
                count: Number(value),
            }))
        })
                                ...
                                >
                                    ...
                                    <div className="flex items-center ml-auto">
                                        <Button
                                            ...
                                            onClick={() => {
                                                const currentValue = form.getValues(`levelId.${level.id}`) || 0
                                                if (currentValue > 0) {
                                                    form.setValue(`levelId.${level.id}`, currentValue - 1)
                                                }
                                            }}
                                        >
